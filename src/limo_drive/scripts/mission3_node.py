...
class DWAObstacleAvoid(object):
    def __init__(self):
        ...
        # ★ mission3 전용 enable 플래그
        #   FSM에서 /mission3_enable True일 때만 연산/발행
        self.enabled = True  # 단독 테스트용으로 기본 True, FSM쓰면 False로 param 내려도 됨.

        # ROS IO (토픽 이름은 파라미터로도 바꿀 수 있게)
        scan_topic   = rospy.get_param("~scan_topic",   "/scan")
        odom_topic   = rospy.get_param("~odom_topic",   "/odom")
        # ★ mission3 전용 cmd 토픽
        cmd_topic    = rospy.get_param("~cmd_topic",    "/cmd_vel_obstacle_m3")
        # ★ mission3 전용 enable 토픽
        enable_topic = rospy.get_param("~enable_topic", "/mission3_enable")
        ...
        rospy.Subscriber(enable_topic, Bool,     self.cb_enable, queue_size=1)
    ...
    def spin(self):
        ...
        while not rospy.is_shutdown():
            ...
            # ★ 게이팅: mission3_enable == False 이면 연산 싹 스킵 + 정지 명령
            if not self.enabled:
